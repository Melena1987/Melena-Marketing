...
const PartnerSection: React.FC = () => {
  ...
  const handlePartnerToggle = (id: string, e: React.MouseEvent) => {
    // We only apply this logic if the device doesn't support hover (touch devices)
    // or if the user clicks a non-active card.
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {PARTNERS_STRUCTURE.map((partner) => (
            <div key={partner.id} className="animate-fade-in-up" style={{ animationDelay: '100ms' }}>
              <PartnerCard 
                id={partner.id}
